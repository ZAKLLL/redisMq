...
public class ProxyMessageDecoder extends LengthFieldBasedFrameDecoder {
    ...
    @Override
    protected TransferMsg decode(ChannelHandlerContext ctx, ByteBuf in2) throws Exception {
        ...
        transferMsg.setSerialNumber(sn);

        transferMsg.setType(type);

        byte uriLength = in.readByte();
        byte[] uriBytes = new byte[uriLength];
        in.readBytes(uriBytes);
        transferMsg.setUri(new String(uriBytes));
